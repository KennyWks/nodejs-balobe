...
exports.CreateCartsModel = (data) => {
    return new Promise((resolve, reject) => {
        runQuery(`
INSERT INTO carts(id_user,id_item,name_item,total_item,total_price,is_check_out) values('${data.id_user}','${data.id_item}','${data.name_item}','${data.total_item}','${data.total_price}','${data.is_check_out}')`, (err, result) => {
            if (err) {
...
exports.GetAllCartsModel = (params, id_user) => {
    return new Promise((resolve, reject) => {
        ...
        runQuery(`
            ...
            SELECT * FROM carts WHERE id_user=${id_user} ${condition}
            `, (err, result) => {
            if (err) {
                return reject(new Error(err));
            }
    ...
    return new Promise((resolve, reject) => {
        runQuery(`SELECT * FROM carts WHERE id=${id_carts}`, (err, result) => {
            ...
            runQuery(`UPDATE carts SET is_check_out = 1 WHERE id = ${id_carts}`, (err, result) => {
                ...
                runQuery(`INSERT INTO transaction(id_user,list_item,total_price) values('${data.id_user}','${data.list_item}','${data.total_price}')`, (err, result) => {
                    if (err) {
                        return reject(new Error(err));
                    }
                    return resolve(result);
