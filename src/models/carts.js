...
exports.GetAllCartsModel = (params, id_user) => {
    return new Promise((resolve, reject) => {
        ...
        const select = "`carts`.`id`, `carts`.`id_user`, `carts`.`id_item`, `carts`.`total_item`, `carts`.`total_price`, `items`.`name` as name_item, `items`.`price`, `items`.`quantity`, `items`.`image`, `pelapak`.`name` as name_pelapak";
