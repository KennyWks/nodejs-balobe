...
exports.SignupController = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      html: `<p>Click this <a href="${
        process.env.APP_ENV === "development" ? "http://localhost:5000" : urlApp
      }/auth/confirmAccount?id_user=${id_user}&verify_code=${encodeURI(
        hashUsername
      )}">link</a> for confirm</p>`,
    };
...
exports.ConfirmAccountController = async (req, res) => {
  try {
    ...
    const user = await GetUserByIdModel(req.query.id_user);
    if (user[1].length > 0) {
        ...
        const mailOptions = {
          ...
          html: `<p>Click this <a href="${
            process.env.APP_ENV === "development"
              ? "http://localhost:5000"
              : urlApp
          }/auth/confirmPass?id_user=${idUser}&verify_code=${encodeURI(
            hashEmail
          )}">link</a> for change your password</p>`,
        };
...
exports.ConfirmPassController = async (req, res) => {
  try {
    ...
    const dataUser = await GetUserByIdModel(req.query.id_user);
            ...
            data: {
              link: `${
                process.env.APP_ENV === "development"
                  ? "http://localhost:5000"
                  : urlApp
              }/auth/updatePass/${req.query.id_user}`,
            },
          });
        } else {
